...
def main():
    ...
    """
    res = requests.get('http://localhost:3001/graphql', data={'query': f'{{ solveCubic(y: {y}, a: {a}, b: {b} c: {c}, d: {d}){{ roots yIntercept }} }}'}) 
    res = json.loads(res.content.decode())['data']['solveCubic']
