...
def main():
    ...
    """
    res = json.loads(requests.get('http://localhost:3001/graphql', data={'query': f'{{ getPolyReg(x: {x}, y: {y}, highestDegree: {highestDegree}){{ coefficients equation scores }} }}'}, headers={'X-API-Key': '1234'}).content.decode())
