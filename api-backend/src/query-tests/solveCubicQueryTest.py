...
def main():
    ...
    """
    res = json.loads(requests.get('http://localhost:3001/graphql', data={'query': f'{{ solveCubic(y: {y}, a: {a}, b: {b} c: {c}, d: {d}){{ roots yIntercept }} }}'}, headers={'X-API-Key': '1234'}).content.decode())
