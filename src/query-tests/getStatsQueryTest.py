...
def main():
    ...
    """
    res = json.loads(requests.get('http://localhost:3001/graphql', data={'query': f'{{ getStats(values: {values}){{ mean median q1 q3 iqr popsd sampsd skewness max min }} }}'}, headers={'X-API-Key': '1234'}).content.decode())
